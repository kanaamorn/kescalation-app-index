<template>
  <div id="main" class="w-[1000px] min-h-screen m-auto overflow-x-auto" style="font-family: 'Sarabun', sans-serif;">
    <div v-if="true">
      <div class="flex flex-row">
        <div class="basis-[130px] p-[6px] text-blue-600 text-lg">วันเสนอราคา</div>
        <div>
      ...
      <div class="flex flex-row mt-2">
        <div class="basis-[130px] p-[6px] text-blue-600 text-lg">
          วันสิ้นสุดสัญญา
        </div>
        <div>
          <Datepicker @selected-date="selectDate" :time-range-a="timeRangeA" :time-range-z="timeRangeZ"
      ...
      <div class="">
        <div class="flex flex-row mt-0 ml-8 text-green-600">
          <div class="shrink-0 basis-[610px] text-right">ผลรวม เงินชดเชย&nbsp;&nbsp;&nbsp;</div><div class="shrink-0 basis-[170px] text-right"><span class="border-b-4 border-green-600 border-double">{{ moneyReturn }}</span></div>
        </div>
      </div>
      <br />
      <div
    ...
    <br>
    <br>
    <div class="overflow-hidden border-2 border-gray-200 rounded ">
      <button class="w-1/2 px-1 py-3 text-xl transition bg-gray-200 border-r border-gray-200 cursor-pointer hover:bg-gray-300 hover:text-white" :class="{'bg-gray-800 text-white hover:bg-gray-800': isCalTab}" @click="isCalTab = true">รายการคำนวน</button>
      <button class="w-1/2 px-1 py-3 text-xl transition bg-gray-200 border-l border-gray-200 cursor-pointer hover:bg-gray-300 hover:text-white" :class="{'bg-gray-800 text-white hover:bg-gray-800': !isCalTab}" @click="isCalTab = false">เอกสารคำนวน ค่า K</button>
    </div>
    <br />
    <div v-if="true" id="report" v-show="isCalTab">
      <div id="summary" v-if="kValue" class="relative">
        ...
        <div class="overflow-auto">
          <table id="TableToExport" class="mx-auto border border-collapse table-fixed border-slate-400">
            <tr class="text-center">
              ...
              <td class="border border-slate-300 p-2 w-[370px]">สูตรค่า K</td>
            ...
            <template v-for="(v, i) in timePay" :key="v.id">
              <tr>
                <td colspan="5" class="p-2 border border-slate-300">ส่งงวดงาน ที่ {{ i + 1 }} <span v-if="v.time">วันที่ {{ thaiDate(v.time)[3] }}</span></td>
              </tr>
              <tr v-for="(k, ki) in v.kValue" :key="k.id">
                <template v-if="k.kIndex">
                  <td class="p-2 border border-slate-300">{{ ki + 1 }}</td>
                  <td class="p-2 truncate border border-slate-300">{{ k.kIndex.replace(/_/g,'.') + " " + kFom[k.kIndex][2] }}</td>
                  <td class="p-2 text-right border border-slate-300">{{ ((+k.money)/10000).toLocaleString("th-TH", { maximumFractionDigits: "4", minimumFractionDigits: "4" }) }}</td>
            ...
            <tr>
              ...
              <td class="p-2 text-right border border-slate-300"><span class="border-b-4 border-double border-gray-950">{{ moneyReturn }}</span></td>             
            </tr>
          </table>
        </div>
      ...
      <div class="relative">
        <div class="py-3 text-xl text-center text-black">รายการคำนวน</div>
        <br>
        <pre id="clip" class="">
          <code id="wordDoc" class="" v-html="reports[1]"></code>
        </pre>
        <button
          class="absolute top-12 right-2 z-10 p-[2px] border rounded border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white hover:cursor-pointer"
          @click="copy(source)">
          <Icon name="ion:copy-outline" /> copy
        </button>
        <button
          class="absolute top-2 right-2 z-10 p-[2px] border rounded border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white hover:cursor-pointer"
          @click="exportToDoc">
    ...
    <div id="downloadPdf" v-show="!isCalTab">
...
function calK(Do, Dt, K, is) {
  ...
  txt.push(`\t\t${str}`);
  txt.push(
    `\t\t\t<span style="color: #FF8C00; font-size :0.8rem;">(กำหนดให้ใช้ทศนิยม 3 ตำแหน่ง ทุกขั้นตอนโดยไม่มีการปัดเศษ)</span>`
  );
  str = "";
  str += "K = " + kFom[K][0][0];
  for (let i = 0; i < kFom[K][1].length; i++) {
...
var reports = computed(() => {
  ...
  txt.push(
    `<span class="text-lg text-blue-600 ">วันเสนอราคา          ${thaiDate(st)[3]
    }</span>`
  );
  txt.push(
    `<span class="text-lg text-blue-600">วันสิ้นสุดสัญญา        ${thaiDate(ft)[3]
    }</span>`
  );
  txt.push("");
  txt.push("<hr>");
  for (let i = 0; i < timePay.value.length; i++) {
    ...
    txt.push(
      `<span class="text-lg text-blue-600">วันส่งมอบงาน งวดที่${i + 1}   ${thaiDate(pt)[3]
      }</span>`
    );
    if (pt <= ft) {
      var isInTime = true;
    ...
    for (let j = 0; j < timePay.value[i].kValue.length; j++) {
      ...
      txt.push(
        `\t<span class="text-lg text-green-600">รายการที่${j + 1}  ${K.replace(
          /_/g,
          "."
        )}  ${kFom[K][2]}</span>`
      );
